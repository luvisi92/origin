 ...
 */
@Controller
@RequestMapping("/QqsRegions")
public class QqsRegionsController extends BaseController{
	
	@Autowired
	private QqsRegionsService service;

	@RequestMapping(value = "/index.html")
	public String index(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
        QueryFilter filter = new QueryFilter(request);
        model.put("pager", service.findPager(filter).getReturnObj());
		return "/QqsRegions/index";
	}
	
	@RequestMapping(value = "/add.html")
	public String add(HttpServletRequest request, HttpServletResponse response) {
		return "/QqsRegions/add";
	}
	
	@RequestMapping(value = "/save.json", method = RequestMethod.POST)
	@ResponseBody
	public ResponseData save(ModelMap model, @ModelAttribute("entity") QqsRegions entity, 
						HttpServletRequest request, HttpServletResponse response) {
        try{
			Map<String,Object> errors = FormValidatorManager.validate("saveQqsRegionsConfig", request);
	        if (errors.size() != 0)
	        {
		    ...
		    if (!result.isSuccess()) {
	            ...
	            if (e.getRootCause().getMessage().toUpperCase().contains("PRIMARY")){
	                result.addErrormsg("region_id  重复");
	            }
	            return new ResponseData(false, result.getErrormsg());
        } catch(Exception e)
        {
    ...
    public String detail(Long regionId, HttpServletRequest request, HttpServletResponse response) {
    	QueryFilter filter = new QueryFilter();
        filter.put("regionId", regionId);
        request.setAttribute("entity", service.findOne(filter).getReturnObj());
        return "/QqsRegions/detail";
    }

	@RequestMapping(value = "/edit.html")
    public String edit(Long regionId, HttpServletRequest request, HttpServletResponse response) {
    	QueryFilter filter = new QueryFilter();
        filter.put("regionId", regionId);
        request.setAttribute("entity", service.findOne(filter).getReturnObj());
        return "/QqsRegions/edit";
    }
    
    @RequestMapping(value = "/update.json", method = RequestMethod.POST)
    @ResponseBody
	public ResponseData update(ModelMap model, @ModelAttribute("entity") QqsRegions entity, 
						HttpServletRequest request, HttpServletResponse response) {
        try{
			Map<String,Object> errors = FormValidatorManager.validate("saveQqsRegionsConfig", request);
	        if (errors.size() != 0)
